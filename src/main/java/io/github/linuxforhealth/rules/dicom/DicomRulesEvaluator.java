...
public class DicomRulesEvaluator extends RulesEvaluator {
  private static final Logger LOGGER = LoggerFactory.getLogger(DicomRulesEvaluator.class);
  ...
  public DicomRulesEvaluator(String ruleDefinition, Format format) {
  ...
  /**
   * Evaluates rules for each DICOM file in the study folder.
   * 
   * @param studyDir -- Expects the file object to be a directory containing DICOM files.
   * @return Map of result, where key is the DICOM file name and value is result of rules evaluation
   *         {@link RulesEvaluationResult} for that DICOM file.
   * @throws IOException
   */
  ...
  /**
   * Evaluates rules on JSON representation of single DICOM instance.
   * 
   * @param dicomJson
   * @return result {@link RulesEvaluationResult}
   * @throws IOException
   */
